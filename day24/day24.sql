...
-- 查询江西省的区县,效率从低到高
SELECT city.aid ,city.atitle from areas as city , areas as p WHERE city.pid = p.aid AND p.atitle = '江西省';
SELECT city.aid ,city.atitle from areas as city INNER JOIN areas as p WHERE city.pid = p.aid AND p.atitle = '江西省';
SELECT city.aid ,city.atitle from areas as city INNER JOIN areas as p ON city.pid = p.aid WHERE p.atitle = '江西省';
SELECT city.aid ,city.atitle from areas as city INNER JOIN areas as p ON city.pid = p.aid AND p.atitle = '江西省';

SELECT city.aid ,city.atitle from areas as city INNER JOIN areas as p ON city.pid = p.aid AND p.atitle = '赣州市';
...
insert into classes values(10,'python_10期');
-- 列级子查询
-- •查询还有学生在班的所有班级名字
-- a.找出学生表中所有的班级 id
-- b.找出班级表中对应的名字
select name from classes where id in (select DISTINCT cls_id from students);
select name from classes where name not in(select  DISTINCT c.name  from classes c inner join students s on c.id = s.cls_id); 
